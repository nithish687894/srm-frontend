...
export default function MatrixMarks({ marks, handleSync, isSyncing }: any) {
  ...
  return (
    <div style={{ background: MATRIX_COLORS.bg, minHeight: "100vh", color: MATRIX_COLORS.text, fontFamily: "'JetBrains Mono', monospace", paddingBottom: "120px" }}>
      ...
      <header style={{ padding: "60px 24px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: 'sticky', top: 0, background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(20px)', zIndex: 100, borderBottom: `1px solid ${MATRIX_COLORS.border}` }}>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
              <span style={{ fontSize: '7px', fontWeight: 900, color: MATRIX_COLORS.accent, opacity: 0.8 }}>01</span>
              <span style={{ fontSize: '7px', fontWeight: 900, color: MATRIX_COLORS.accent, opacity: 0.8 }}>10</span>
            </div>
            <span style={{ fontSize: "11px", fontWeight: 900, color: MATRIX_COLORS.accent, textTransform: "uppercase", letterSpacing: "0.4em" }}>ENCRYPTED_SCORES</span>
          </div>
        </div>
      ...
      <main style={{ padding: "24px 20px" }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
           {marks?.map((m: any, i: number) => {
              ...
              return (
                <div key={i} style={{ background: "rgba(168,194,0,0.01)", border: `1px solid ${MATRIX_COLORS.border}`, borderRadius: '28px', padding: '24px', position: 'relative', overflow: 'hidden' }}>
                   <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundImage: 'linear-gradient(rgba(168,194,0,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(168,194,0,0.02) 1px, transparent 1px)', backgroundSize: '24px 24px', pointerEvents: 'none' }} />
                   
                   <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', position: 'relative' }}>
                      <div style={{ flex: 1, paddingRight: '16px' }}>
                         <div style={{ fontSize: '9px', color: MATRIX_COLORS.accent, background: 'rgba(168,194,0,0.1)', padding: '4px 10px', borderRadius: '8px', display: 'inline-block', marginBottom: '12px', fontWeight: 800 }}>{m.courseCode || m.code}</div>
                         <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#fff', margin: 0, lineHeight: 1.3, textTransform: 'uppercase', letterSpacing: '0.02em' }}>{m.title}</h3>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                         <div style={{ fontSize: '36px', fontWeight: 900, color: MATRIX_COLORS.accent, lineHeight: 1 }}>{scored.toFixed(1)}</div>
                         <div style={{ fontSize: '11px', color: MATRIX_COLORS.sub, marginTop: '6px', fontWeight: 800 }}>/{max}</div>
                      </div>
                   </div>
                   <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '12px', position: 'relative' }}>
