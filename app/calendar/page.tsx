            ...
            <>
              ...
              <div style={{ ...topCardStyle, borderRadius: "24px", padding: "24px", marginBottom: "32px", display: "flex", flexDirection: "column", gap: "8px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ padding: "4px 12px", background: false ? "rgba(255,255,255,0.12)" : "#a8c200", color: false ? "#fff" : "#000000", fontWeight: "bold", fontSize: "11px", textTransform: "uppercase", borderRadius: "99px", letterSpacing: "0.1em" }}>
                    {todayStr}
                  </div>
                  <div style={{ fontSize: "12px", color: false ? "#9EC5FF" : "#a8c200", letterSpacing: "0.2em", textTransform: "uppercase", fontWeight: "bold" }}>
                    Day Order
                  </div>
                </div>
                
                <div style={{ display: "flex", alignItems: "baseline", justifyContent: "flex-end", gap: "16px", marginTop: "16px" }}>
                  <div style={{ fontSize: "100px", fontWeight: 900, color: false ? "#fff" : "#a8c200", lineHeight: 0.85 }}>
                    {isTodayHoliday ? "—" : todayInfo?.dayOrder || "—"}
                  </div>
                  <div style={{ fontSize: "32px", fontWeight: "bold", color: false ? "#8FD3FF" : "#a8c200" }}>
                    {todayDateNum}
                  </div>
                </div>
                ...
                {(["ODD", "EVEN"] as Semester[]).map(s => (
                  <button key={s} onClick={() => { setSem(s); setMonthIdx(0); }}
                    style={{ 
                      ...
                      background: sem === s ? (false ? "linear-gradient(90deg, #1A75FF, #6B33FF)" : "#ffffff") : "transparent", 
                      color: sem === s ? "#ffffff" : (false ? "#7E88B6" : "#555555"), 
                      border: sem === s ? "none" : `1px solid ${false ? "rgba(255,255,255,0.12)" : "#333333"}`, 
              ...
              {isLoading ? (
                ...
                <div style={{ background: false ? "linear-gradient(180deg, rgba(36,40,84,0.48), rgba(19,20,48,0.45))" : "#1a1a1a", borderRadius: "24px", padding: "24px", position: "relative", border: false ? "1px solid rgba(255,255,255,0.08)" : "none", backdropFilter: false ? "blur(14px)" : "none" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
                    ...
                    <div style={{ display: "flex", gap: "12px" }}>
                      ...
                      <button onClick={() => setMonthIdx(semMonths.findIndex(m => m.days.some((d: AnyValue) => d.isoDate === todayIso)) || 0)} style={{ background: "none", border: "none", color: false ? "#8FD3FF" : "#ffffff", fontSize: "16px", cursor: "pointer", display: "flex", alignItems: "center" }}>◎</button>
                    ...
                    {gridCells.map((cell, i) => {
                      ...
                      return (
                        <div key={i} 
                          ...
                          {isToday ? (
                            <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: false ? "linear-gradient(135deg,#1A75FF,#6B33FF)" : "#6366f1", color: "#ffffff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", fontWeight: "bold", boxShadow: false ? "0 8px 18px rgba(26,117,255,0.45)" : "0 4px 12px rgba(99, 102, 241, 0.4)" }}>
                              {cell.dateNum}
                            </div>
                          ) : (
                          ...
                          {cell.isHoliday && (
                            <div style={{ position: "absolute", bottom: "-6px", width: "4px", height: "4px", borderRadius: "50%", background: false ? "#F97316" : "#ff3b3b" }} />
                          )}

                          {selectedHoliday?.isoDate === cell.isoDate && (
                            <div style={{
                              position: "absolute", bottom: "100%", left: "50%", transform: "translateX(-50%)",
                              background: false ? "rgba(22, 25, 56, 0.95)" : "#2a2a2a", border: false ? "1px solid rgba(255,255,255,0.12)" : "1px solid #444", padding: "12px", borderRadius: "12px",
              ...
              {current && (
                <div style={{ marginTop: "40px", animation: "slideUp 0.4s ease-out forwards" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
                    <div style={{ fontSize: "12px", fontWeight: 800, color: false ? "#9EC5FF" : "#a8c200", textTransform: "uppercase", letterSpacing: "0.15em" }}>
                      {current.name} — Events
                    </div>
                    <div style={{ padding: "8px 20px", background: false ? "rgba(26,117,255,0.15)" : "#333", borderRadius: "12px", fontSize: "11px", fontWeight: 800, color: false ? "#8FD3FF" : "#fff", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                      Events
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                    {current.days.filter((d: AnyValue) => d.event).length === 0 ? (
                      ...
                      current.days.filter((d: AnyValue) => d.event).map((d: AnyValue, idx: number) => {
                        ...
                        return (
                          <div key={idx} style={{ 
                            background: false ? "rgba(255,255,255,0.03)" : "#1c1c1c", 
                            borderRadius: "20px", padding: "16px", 
                            display: "flex", alignItems: "center", gap: "20px",
                            borderLeft: isHoliday ? `4px solid ${false ? "#F97316" : "#ff3b3b"}` : "4px solid transparent",
                            border: false ? "1px solid rgba(255,255,255,0.05)" : "none",
                            transition: "transform 0.2s"
                          }}>
                            <div style={{ 
                              width: "44px", height: "44px", borderRadius: "14px", 
                              background: isHoliday ? (false ? "rgba(249,115,22,0.1)" : "rgba(255,59,59,0.1)") : "rgba(255,255,255,0.05)",
                              display: "flex", alignItems: "center", justifyContent: "center",
                              fontSize: "18px", fontWeight: 900, color: isHoliday ? (false ? "#F97316" : "#ff3b3b") : "#fff"
                            }}>
                              {d.dateNum}
                            </div>
                            <div style={{ flex: 1 }}>
